...
import CitesModel from '../models/citys'
...
class CityHandle extends AddressComponent{
		...
		try{ 
			switch (type){
				case 'guess': 
					...
					cityInfo = await CitesModel.cityGuess(city);
					break;
				case 'hot': 
					cityInfo = await CitesModel.cityHot();
					break;
				case 'group': 
					cityInfo = await CitesModel.cityGroup();
					break;
				default: 
					res.json({
	...
	async getCityById(req, res, next){
		...
		try{
			const cityInfo = await CitesModel.getCityById(cityid);
			res.send(cityInfo);
		}catch(err){
			res.send({
				name: 'ERROR_DATA',
